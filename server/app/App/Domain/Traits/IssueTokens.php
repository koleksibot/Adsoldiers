...
trait IssueTokens
{
    ...
    {
        ${$this->process} = $user->{$this->process}()->where(['completed_at' => null, ['created_at', '>=', now()->subDays(config("adsoldiers.{$this->process}.expiration"))->format('Y-m-d H:i:s')]]);
        if ($token) {
    ...
    {
        return $user->{$this->process}()->with('user')->firstOrCreate(['completed_at' => null, ['created_at', '>=', now()->subDays(config("adsoldiers.{$this->process}.expiration"))]], ['created_at' => now(), 'token' => Str::random(32)]);
    }
    ...
    {
        return $this->where(['completed_at' => null, ['created_at', '<=', now()->subDays($days ?? config("adsoldiers.{$this->process}.expiration"))]])->delete();
    }
}
